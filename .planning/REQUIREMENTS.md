...
### Branding
- [x] **BRND-01**: Steco logo (public/logo-hvit.png) replaces the "StecoPro" wordmark in app shell and login page

### Simulator Calibration
- [x] **SIMU-09**: Simulator calibrated to real plant: ~10 t/h nominal (30,000 t/yr, 2 shifts, 80% design uptime), bales ~1 t (~135 bales/day at 90% sim availability), fraction mix 50% deink / 10% OCC / 8% tetra / 32% miks, bunker motor 11 kW drawing 10-15 A loaded and 4-6 A empty (empty threshold ~8 A); OEE performance nominal aligned

### Dashboard
- [x] **DASH-09**: Dashboard shows a bar chart of bales produced today per fraction

### Inventory
- [x] **INVT-01**: Bale production continuously fills per-fraction inventory (stock = produced − shipped)
- [x] **INVT-02**: Users register outbound shipments (fraction, bale count) which reduce stock; shipment history visible
- [x] **INVT-03**: Dashboard widget shows current bale stock per fraction

### Traceability (v1.1)

| Req ID | Phase | Status |
|--------|-------|--------|
| BRND-01 | Phase 6 | Complete |
| SIMU-09 | Phase 6 | Complete |
...
### PWA & Notifications
- [x] **PWAS-01**: App is installable as PWA (manifest, icons, service worker) with logged-in registration
- [x] **NOTI-01**: Push notifications on fault stops and 'Bunker tom' (never planned stops), with VAPID web-push pipeline, per-device subscriptions, throttling and dead-subscription pruning
- [x] **NOTI-02**: /varsler page lists recent notifiable stops (48h) with acknowledgement status and a push enable/disable toggle

### In-shift Reporting
- [x] **REPT-01**: Operators acknowledge alarms; acknowledgements visible to the team (idempotent per user+stop)
- [x] **REPT-02**: Operators comment on stops and optionally correct the stop reason
- [x] **REPT-03**: Operators write free-text shift notes (logbook) per plant
- [x] **REPT-04**: Photos can be attached to stop comments and shift notes (mobile camera capture, ≤10MB, tenant-scoped serving)

### Traceability (v1.2)

| Req ID | Phase | Status |
|--------|-------|--------|
| PWAS-01 | Phase 7 | Complete |
| NOTI-01 | Phase 7 | Complete |
| NOTI-02 | Phase 7 | Complete |
